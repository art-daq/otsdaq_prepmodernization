...
//========================================================================================================================
NimStreamConsumer::NimStreamConsumer(std::string supervisorApplicationUID, std::string bufferUID,
                                     std::string processorUID, const ConfigurationTree& theXDAQContextConfigTree,
                                     const std::string& configurationPath)
    : WorkLoop(processorUID),
      VisualVConsumer(supervisorApplicationUID, bufferUID, processorUID, theXDAQContextConfigTree, configurationPath)
...
bool NimStreamConsumer::workLoopThread(toolbox::task::WorkLoop* workLoop) {
  ...
  while ((data_str.length() >= 64)) {
    ...
    newPt.timestamp = (data_int & 0x0000FFFFFFFF0000) >>
                      2;  // One Quadword, Mask out header and state data #TODO VERIFY THAT TIMESTAMP IS 32b
...
void NimStreamConsumer::slowRead(void) {
  ...
  if (DataConsumer::read(data_, header_) < 0) {
    ...
    __COUT__ << "Size of data_; " << data_.length() << " Size of data_str: " << data_str.length() << __E__;
...
std::string NimStreamConsumer::getNext(std::map<std::string, std::string> args) {
  ...
  // Find the closest timestamp and return up to "count" timestamp points via vector for the VisualDataManager to
  // interpret and send to the js frontend
  if (timeline.size() > 0) {
