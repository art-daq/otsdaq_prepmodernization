...
FENIMPlusInterfaceV1::FENIMPlusInterfaceV1(
...
void FENIMPlusInterfaceV1::configure(void)
{
	...
	OtsUDPHardware::write(writeBuffer);
	OtsUDPFirmwareCore::writeAdvanced(
	    writeBuffer, 0x18001, nimEnables_.to_ulong());  // disable everything
	OtsUDPHardware::write(writeBuffer);
	...
	try
	{
		...
		{
			outputChannelSourceSelect = selectOutputChannelSource(
			    theXDAQContextConfigTree_.getNode(theConfigurationPath_)
			...
			OtsUDPFirmwareCore::writeAdvanced(
			    ...
			    outputWidth | (uint64_t(outputWidth ? 1 : 0 /*enable*/) << 63));
			OtsUDPHardware::write(writeBuffer);
		...
		// setup burst data blocks
		{
			...
			{  // enable timestamp counter if using the scope
				OtsUDPFirmwareCore::writeAdvanced(
				    writeBuffer, 0x1800F, 0x2);  // Enable Timestamp counter for scope
				OtsUDPHardware::write(writeBuffer);
			}
			else
			{
				OtsUDPFirmwareCore::writeAdvanced(
				    writeBuffer, 0x1800F, 0);  // Enable Timestamp counter for scope
				OtsUDPHardware::write(writeBuffer);
			}
		}
		...
		OtsUDPFirmwareCore::writeAdvanced(
		    writeBuffer, 0x7, coincidenceLogicWord);  // setup selection logic
		OtsUDPHardware::write(writeBuffer);
		...
		OtsUDPFirmwareCore::writeAdvanced(
		    writeBuffer, 0x6, sel_ctl_register_);  // re-enable selection logic
		OtsUDPHardware::write(writeBuffer);
...
void FENIMPlusInterfaceV1::start(std::string runNumber)
{
	...
	OtsUDPFirmwareCore::writeAdvanced(
	    writeBuffer,
	    0x18000,
	    nimResets_.to_ulong());  // reset everything (counters, e.g. sig_log and
	                             // sig_norm/cms1/cms2 counters, and vetos/ps)
	OtsUDPHardware::write(writeBuffer);

	nimResets_.reset();  // reset all bits to 0
	nimResets_.set(
	    5);  // keep always signal generator in reset unless it is needed in running!
	OtsUDPFirmwareCore::writeAdvanced(
	    writeBuffer,
	    0x18000,
...
void FENIMPlusInterfaceV1::stop(void)
{
	...
	OtsUDPFirmwareCore::writeAdvanced(
	    writeBuffer,
	    /*address*/ 0x4,
	    /*data*/ 0x33);  // only reset signorm  0x33); //reset output channel blocks
	                     // synchronously
	OtsUDPHardware::write(writeBuffer);
}
...
bool FENIMPlusInterfaceV1::running(void)
{
	...
	try
	{
		...
		for(unsigned char channelCount = 2; channelCount <= 2; --channelCount)
		{
			...
			{
				enable40MHzMask = optionalLink
				                      .getNode("EnableClockMaskTriggerOutput" +
				                               outChannelNames_[channelCount])
				                      .getValue<bool>();
				gateChannelVetoSel = optionalLink
		...
		if(usingOptionalParameters)
		{
			...
			// THIS IS A SUPER PATCH TO RUN JIM FREEMAN STUFF! I have put false on all
			// places where the variable SignalGeneratorEnable is used!
		...
		if(true && usingOptionalParameters &&
		   optionalLink.getNode("SignalGeneratorEnable").getValue<bool>())
		{
		...
		else
		{
			...
			OtsUDPFirmwareCore::writeAdvanced(
			    writeBuffer,
			    0x6,
			    sel_ctl_register_);  // enable sig mod block and restore original register
			                         // value
			OtsUDPHardware::write(writeBuffer);
		}
	}
...
//========================================================================================================================
void FENIMPlusInterfaceV1::changeDACLevel(const std::string& channelName,
...
//========================================================================================================================
void FENIMPlusInterfaceV1::sendPatternTrigger(uint64_t    patternToSend,
