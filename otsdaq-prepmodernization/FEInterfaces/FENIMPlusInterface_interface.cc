...
void FENIMPlusInterface::configure (void)
{
	...
	OtsUDPFirmwareCore::writeAdvanced (writeBuffer, 0x3, (usingOptionalParams ? (optionalLink.getNode ("UseExternalClock").getValue<bool> () ? 1 : 0) : 0) | (((usingOptionalParams ? (optionalLink.getNode ("ExternalClockSource").getValue<unsigned int> () ? (optionalLink.getNode ("ExternalClockSource").getValue<unsigned int> () - 1) /*subtract 1 to normal index*/ : 0 /*default to NW-FMC-PTA*/) : 0) & 0x7) << 4)  //Choosing external clock source := 1-4 (front panel NIM-input A-D), 0 (NW-FMC-PTA clk source)

	);  //Choosing external := 1, internal := 0

	unsigned val = (usingOptionalParams ? (optionalLink.getNode ("UseExternalClock").getValue<bool> () ? 1 : 0) : 0) |
	               (((usingOptionalParams ? (optionalLink.getNode ("ExternalClockSource").getValue<unsigned int> () ? (optionalLink.getNode ("ExternalClockSource").getValue<unsigned int> () - 1) /*subtract 1 to normal index*/ : 0 /*default to NW-FMC-PTA*/) : 0) & 0x7) << 4);
	...
	try
	{
		...
		OtsUDPFirmwareCore::writeAdvanced (writeBuffer,
		                                   0x107,
		                                   (usingOptionalParams ? optionalLink.getNode ("40MHzClockDelay").getValue<unsigned int> () : 0));  //chooses a 40MHz clock phase relative to the accelerator clock (in increments of ~3ns)
		OtsUDPHardware::write (writeBuffer);
		...
		OtsUDPFirmwareCore::writeAdvanced (writeBuffer,
		                                   ...
		                                   (usingOptionalParams ? optionalLink.getNode ("ClockMaskSetup").getValue<unsigned int> () : 0));  //chooses a section of 40MHz clock
		OtsUDPHardware::write (writeBuffer);
	...
	try
	{
		...
		{
			outputChannelSourceSelect = theXDAQContextConfigTree_.getNode (theConfigurationPath_).getNode ("TriggerInput" + channelName).getValue<unsigned int> ();  //0: sig_log   or    1: sig_norm/ch0
			...
			if (usingOptionalParams)
			{
				...
				outputTimeVetoDuration = optionalLink.getNode ("TimeVetoTriggerOutput" +
				                                               channelName)
				                             .getValue<unsigned int> ();  //0 ignores time veto, units of 3ns (NIM+X) or 6ns (NIM+)
				outputPrescaleCount = optionalLink.getNode ("PrescaleTriggerOutput" +
				                                            channelName)
				                          .getValue<unsigned int> ();
				outputBackpressureSelect = optionalLink.getNode ("BackpressureTriggerOutput" +
				                                                 channelName)
				                               .getValue<bool> ();
				gateChannelVetoSel[channelCount] = optionalLink.getNode ("VetoSourceTriggerOutput" +
				                                                         channelName)
				                                       .getValue<int> ();
			...
			OtsUDPFirmwareCore::writeAdvanced (writeBuffer, 0x106 + channelCount * 0x100, outputWidth | (uint64_t (outputWidth ? 1 : 0 /*enable*/) << 63));  //set output channel long width count
			OtsUDPHardware::write (writeBuffer);
		...
		for (const auto &channelName : channelNames)
		{
			...
			OtsUDPHardware::write (writeBuffer);
			__CFG_COUT__ << "Mux value for output channel " << channelName << " is " << outputMuxSelect << ", written to 0x" << std::hex << (channelCount == 0 ? 0x5 : (0x18013 + channelCount - 1)) << std::dec << std::endl;
		...
		if (optionalLink.getNode ("SignalGeneratorEnable").getValue<bool> ())
		{
			...
			__CFG_COUT__ << "Configured signal generator with a count of " << sigGenCount << " (0 is continuous output), a high period of " << sigGenHighPer << ", a low period of " << sigGenLowPer << ", and output inversion set to " << sigGenPolarity << std::endl;
	...
	__CFG_COUT__ << "Checking sel_ctl_register_... Expect sel_ctl_register_ = 0x" << std::hex << sel_ctl_register_ << std::dec << std::endl;
...
bool FENIMPlusInterface::running (void)
{
	...
	try
	{
		...
		{
			enable40MHzMask = usingOptionalParams && optionalLink.getNode ("EnableClockMaskTriggerOutput" +
			                                                               outChannelNames[channelCount])
			                                             .getValue<bool> ();
			gateChannelVetoSel = usingOptionalParams ? optionalLink.getNode ("VetoSourceTriggerOutput" + outChannelNames[channelCount]).getValue<int> () : 0;
