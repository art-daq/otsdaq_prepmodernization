...
FENIMPlusInterface::FENIMPlusInterface(const std::string &interfaceUID,
  ...
  registerFEMacroFunction(
      ...
      std::vector<std::string>{"numberOfTriggers", "signalHiDuration", "signalLoDuration"},  // namesOfInputArgs
...
void FENIMPlusInterface::configure(void) {
  ...
  ConfigurationTree optionalLink =
      theXDAQContextConfigTree_.getNode(theConfigurationPath_).getNode("LinkToOptionalParameters");
  ...
  {
    try {
      if ((usingOptionalParams && optionalLink.getNode("EnableClockResetDuringConfigure").getValue<bool>())) {
  ...
  OtsUDPFirmwareCore::writeAdvanced(
      writeBuffer, 0x3,
      (usingOptionalParams ? (optionalLink.getNode("UseExternalClock").getValue<bool>() ? 1 : 0) : 0) |
          (((usingOptionalParams ? (optionalLink.getNode("ExternalClockSource").getValue<unsigned int>()
                                        ? (optionalLink.getNode("ExternalClockSource").getValue<unsigned int>() -
                                           1) /*subtract 1 to normal index*/
                                        : 0 /*default to NW-FMC-PTA*/)
                                 : 0) &
            0x7)
           << 4)  // Choosing external clock source := 1-4 (front panel NIM-input A-D), 0 (NW-FMC-PTA clk source)

  );  // Choosing external := 1, internal := 0

  unsigned val = (usingOptionalParams ? (optionalLink.getNode("UseExternalClock").getValue<bool>() ? 1 : 0) : 0) |
                 (((usingOptionalParams ? (optionalLink.getNode("ExternalClockSource").getValue<unsigned int>()
                                               ? (optionalLink.getNode("ExternalClockSource").getValue<unsigned int>() -
                                                  1) /*subtract 1 to normal index*/
                                               : 0 /*default to NW-FMC-PTA*/)
  ...
  __CFG_COUT__ << "CHOOSING EXTERNAL CLOCK: " << usingOptionalParams << " : "
               ...
               << optionalLink.getNode("ExternalClockSource").getValue<unsigned int>() << std::hex << " : " << val
               << std::dec << std::endl;
  ;
  OtsUDPHardware::write(writeBuffer);
  ...
  {  // debug read
    ...
    OtsUDPFirmwareCore::readAdvanced(writeBuffer, 0x5);  // This can be removed when you want
    ...
    if (!((readQuadWord >> 24) & 0xF)) {
      ...
      // reset clock PLLs
      OtsUDPFirmwareCore::writeAdvanced(writeBuffer, /*address*/ 0x999,
      ...
      OtsUDPHardware::write(writeBuffer);
      usleep(100000);                                                                   // micro seconds
      OtsUDPFirmwareCore::writeAdvanced(writeBuffer, /*address*/ 0x999, /*data*/ 0x8);  // reset phase shift output
                                                                                        // clock
      OtsUDPHardware::write(writeBuffer);
      usleep(100000);                                                                 // micro seconds
      OtsUDPFirmwareCore::writeAdvanced(writeBuffer, /*address*/ 0x999, /*data*/ 0);  // unreset
  ...
  if (1) {
    ...
    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x6, sel_ctl_register_);
    OtsUDPHardware::write(writeBuffer);
  }
  ...
  OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x18000,
                                    nimResets_.to_ulong());  // reset everything (counters and vetos/ps)
  OtsUDPHardware::write(writeBuffer);
  OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x18001, nimEnables_.to_ulong());  // disable everything
  OtsUDPHardware::write(writeBuffer);

  nimResets_.reset();  // set all bits to 0
  nimResets_.set(5);   // hold sig gen in reset (only unreset if enabled)
  OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x18000, nimResets_.to_ulong());  // unreset everything
  OtsUDPHardware::write(writeBuffer);
  ...
  try {
    doWriteDACs = usingOptionalParams && optionalLink.getNode("EnableDACSetupDuringConfigure").getValue<bool>();
  } catch (...) {
    __CFG_COUT__ << "Skipping DAC writing because enable field was not found in tree." << std::endl;
  }
  ...
  if (doWriteDACs)  // works, but temporarily disabling
  {
    ...
    for (const auto &channelName : channelNames) {
      changeDACLevel(channelName, optionalLink.getNode(dacValueField + channelName).getValue<unsigned short>());
    }
  }
  ...
  try {
    ...
    for (const auto &channelName : channelNames)  // setup sig mod for each channel
    {
      ...
      // enableInput = theXDAQContextConfigTree_.getNode(theConfigurationPath_).getNode("EnableInput" +
      ...
      if (logicInput) --logicInput;  // handle non-default indexing
      sel_ctl_register_ |= (logicInput & 0x3) << (channelCount * 2 + 8);
      __CFG_COUT__ << "Logic input " << channelName << " : " << logicInput << " => sel_ctl_register_= " << std::hex
      ...
      if (usingOptionalParams) {
        ...
        // selectRawInput = optionalLink.getNode("DoNotUseDelayAndWidthFor" + channelName).getValue<bool>();
      } else {
        inputModMask = 0x7;  // b111 default value
                             // selectRawInput = false; //default to not raw input
      }

      // selectionRawInputMask |= (selectRawInput << channelCount); //Note! Inverted relative to user input (default to
      // sigmod as input, i.e. not raw input ys)

      invertPolarity =
          usingOptionalParams && optionalLink.getNode("InvertPolarityInput" + channelName).getValue<bool>();

      inputPolarityMask |= ((invertPolarity ? 1 : 0) << channelCount);

      __CFG_COUT__ << "Output word for " << channelName << " is " << std::bitset<64>(inputModMask) << std::endl
                   << " with a delay of " << inputDelay << " and a width of " << inputWidth << std::endl;

      OtsUDPFirmwareCore::writeAdvanced(writeBuffer, /*address*/ ((channelCount + 1) << 8),
                                        /*data*/ 0x3);  // reset channel
      OtsUDPHardware::write(writeBuffer);

      OtsUDPFirmwareCore::writeAdvanced(writeBuffer, ((channelCount + 1) << 8) | 0x2,
                                        inputModMask);  // set sig_mod width and delay
      OtsUDPHardware::write(writeBuffer);

      // OtsUDPFirmwareCore::writeAdvanced(writeBuffer, ((channelCount+1) << 8) | 0x4, enableInput?0:1);
      // //enable/disable channel  OtsUDPHardware::write(writeBuffer);

      ++channelCount;
    }
    __CFG_COUT__ << "Input polarity mask is " << std::bitset<8>(inputPolarityMask) << std::endl;
    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, /*address*/ 0x1800B,
                                      /*data*/ inputPolarityMask);  // setup input polarity
    OtsUDPHardware::write(writeBuffer);

    // sel_ctl_register_ |= (selectionRawInputMask << 4);
    __CFG_COUT__ << " sel_ctl_register_: " << std::bitset<16>(sel_ctl_register_) << std::endl;
    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, /*address*/ 0x6,
                                      /*data*/ sel_ctl_register_);  // setup raw input selection muxes, assume the
                                                                    // logic_control_register was 0 before this. (cleared
                                                                    // at start of configure)
    OtsUDPHardware::write(writeBuffer);
    ...
    OtsUDPFirmwareCore::writeAdvanced(
        ...
        (usingOptionalParams ? optionalLink.getNode("ClockMaskSetup").getValue<unsigned int>()
    ...
    __CFG_COUT__ << "Clock mask setup: " << std::hex << optionalLink.getNode("ClockMaskSetup").getValue<unsigned int>()
                 << std::dec << std::endl;
  ...
  try {
    ...
    for (const auto &channelName : outChannelNames) {
      outputChannelSourceSelect = theXDAQContextConfigTree_.getNode(theConfigurationPath_)
                                      .getNode("TriggerInput" + channelName)
                                      .getValue<unsigned int>();  // 0: sig_log   or    1: sig_norm/ch0
      __CFG_COUT__ << "TriggerInput for " << channelName << " is " << outputChannelSourceSelect << std::endl;
      if (outputChannelSourceSelect)  // if non-default, subtract 1 so choice 1 evaluates to 0, and so on..
        --outputChannelSourceSelect;

      if (usingOptionalParams) {
        outputDelay = optionalLink.getNode("DelayTriggerOutput" + channelName).getValue<unsigned int>();
        outputWidth = optionalLink.getNode("WidthTriggerOutput" + channelName).getValue<uint64_t>();
        ...
        outputTimeVetoDuration =
            optionalLink.getNode("TimeVetoTriggerOutput" + channelName)
                .getValue<unsigned int>();  // 0 ignores time veto, units of 3ns (NIM+X) or 6ns (NIM+)
        outputPrescaleCount = optionalLink.getNode("PrescaleTriggerOutput" + channelName).getValue<unsigned int>();
        outputBackpressureSelect = optionalLink.getNode("BackpressureTriggerOutput" + channelName).getValue<bool>();
        gateChannelVetoSel[channelCount] =
            optionalLink.getNode("VetoSourceTriggerOutput" + channelName).getValue<int>();
        __CFG_COUT__ << "Raw gateChannelVetoSelect for " << channelName << " is " << gateChannelVetoSel[channelCount]
                     << std::endl;
      ...
      OtsUDPFirmwareCore::writeAdvanced(writeBuffer, channelCount == 0 ? 0x4 : (0x18016 + channelCount - 1),
                                        0x33);  // reset output channel block (bits 4/5 are reseting ch1/2)
      OtsUDPHardware::write(writeBuffer);

      OtsUDPFirmwareCore::writeAdvanced(writeBuffer, channelCount == 0 ? 0x2 : (0x18002 + channelCount - 1),
                                        outputModMask);  // set output channel width/delay mask
      OtsUDPHardware::write(writeBuffer);

      OtsUDPFirmwareCore::writeAdvanced(
          writeBuffer, 0x106 + channelCount * 0x100,
          outputWidth | (uint64_t(outputWidth ? 1 : 0 /*enable*/) << 63));  // set output channel long width count
      OtsUDPHardware::write(writeBuffer);

      __CFG_COUT__ << "Output word for " << channelName << " is " << std::bitset<64>(outputModMask) << std::endl;
      __CFG_COUT__ << "Output delay of " << outputDelay << " and an extended width of " << outputWidth << std::endl;

      if (channelCount) {
        writeBuffer.resize(0);
        OtsUDPFirmwareCore::writeAdvanced(writeBuffer, (0x18018 + channelCount - 1),
                                          outputChannelSourceSelect);  // select source (1 := signorm, 0 := siglog)
        OtsUDPHardware::write(writeBuffer);
        __CFG_COUT__ << "Output src select is " << outputChannelSourceSelect << " for " << channelName << std::endl;
      }

      // time veto setup
      writeBuffer.resize(0);
      OtsUDPFirmwareCore::writeAdvanced(writeBuffer, channelCount == 0 ? 0x1801B : (0x18011 + channelCount - 1),
                                        outputTimeVetoDuration);
      OtsUDPHardware::write(writeBuffer);

      __CFG_COUT__ << "Veto count for " << channelName << " is " << outputTimeVetoDuration << " writing to ch "
                   << (channelCount == 0 ? 0x1801B : (0x18011 + channelCount - 1)) << std::endl;
      ...
      __CFG_COUT__ << "Prescaler count for " << channelName << " is " << outputPrescaleCount << " writing to ch "
    ...
    outputBackpressureSelect =
        usingOptionalParams && optionalLink.getNode("EnableBackPressureNwFmcPta1").getValue<bool>();
    backpressureMask |= outputBackpressureSelect << 3;
    outputBackpressureSelect =
        usingOptionalParams && optionalLink.getNode("EnableBackPressureNwFmcPta2").getValue<bool>();
    ...
    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x200, (uint64_t)-1);  // setup burst output mux select
    OtsUDPHardware::write(writeBuffer);
    ...
    for (const auto &channelName : channelNames) {
      outputMuxSelect = theXDAQContextConfigTree_.getNode(theConfigurationPath_)
                            ...
                            .getValue<unsigned int>();
      if (outputMuxSelect)
        --outputMuxSelect;  // default is 0, the actual selection address for all other choices is 1 higher
      ...
      if (usingOptionalParams) {
        outputInvertPolarity =
            usingOptionalParams && optionalLink.getNode("InvertPolarityOutput" + channelName).getValue<bool>();
      ...
      writeBuffer.resize(0);
      OtsUDPFirmwareCore::writeAdvanced(writeBuffer, channelCount == 0 ? 0x5 : (0x18013 + channelCount - 1),
                                        outputMuxSelect);  // setup mux select
      OtsUDPHardware::write(writeBuffer);
      __CFG_COUT__ << "Mux value for output channel " << channelName << " is " << outputMuxSelect << ", written to 0x"
                   << std::hex << (channelCount == 0 ? 0x5 : (0x18013 + channelCount - 1)) << std::dec << std::endl;

      ++channelCount;
    }

    writeBuffer.resize(0);
    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, /*address*/ 0x1800C,
    ...
    if (usingOptionalParams) {
      ...
      std::array<std::string, 2> trigClkOutBankNames = {"TriggerMuxSelectionsBankA", "TriggerMuxSelectionsBankB"};
      for (auto &trigClkOutBankName : trigClkOutBankNames) {
        __COUTV__(trigClkOutBankName);

        ConfigurationTree::BitMap bankSelectionMap = optionalLink.getNode(trigClkOutBankName).getValueAsBitMap();
        ...
        unsigned int col = 0;
        for (; col < 6; ++col)
          trigClkOutBankRegister |= (bankSelectionMap.get(0, col) & 0x3)
                                    << ((bank * 6 + col) * 2);  // put the two bit mux selection in the right place

        for (; col < 9; ++col)
          trigClkOutBankRegister |= (bankSelectionMap.get(0, col) & 0x3)
                                    << ((12 + bank * 3 + col) * 2);  // put the two bit mux selection in the right place

        ++bank;
      }

      OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x200, trigClkOutBankRegister);  // setup burst output mux select
      OtsUDPHardware::write(writeBuffer);
    }
    ...
    // setup burst data blocks
    {
      ...
      unsigned int gateChannelReg =
          ...
          (gateChannelVetoSel[0] << 0);  // nibbles ... 3:= delta-time, 2:= out-ch2, 1:= out-ch1, 0 := out-ch0
      __CFG_COUT__ << "Gate Ch Veto Selections - 2: " << gateChannelVetoSel[2] << std::endl;
      __CFG_COUT__ << "Gate Ch Veto Selections - 1: " << gateChannelVetoSel[1] << std::endl;
      __CFG_COUT__ << "Gate Ch Veto Selections - 0: " << gateChannelVetoSel[0] << std::endl;
      ...
      __CFG_COUT__ << "Gate Ch Veto Selections - Burst: " << gateChannel << std::endl;
      std::bitset<16> gateChannelRegBitset(gateChannelReg);
      __CFG_COUT__ << "Gate Ch Veto Register: " << gateChannelReg << std::endl;
      __CFG_COUT__ << "Gate Ch Veto Register: " << gateChannelRegBitset.to_string() << std::endl;
    ...
    unsigned int coincidenceLogicWord = theXDAQContextConfigTree_.getNode(theConfigurationPath_)
    ...
    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x6, sel_ctl_register_);
    OtsUDPHardware::write(writeBuffer);
    __CFG_COUT__ << " sel_ctl_register_ 1: " << std::bitset<16>(sel_ctl_register_) << std::endl;

    sel_ctl_register_ &= ~(3);  // disable selection logic, take out of reset (i.e. bits 0 and 1 <= 0)
    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x6, sel_ctl_register_);
    OtsUDPHardware::write(writeBuffer);
    __CFG_COUT__ << " sel_ctl_register_ 2: " << std::bitset<16>(sel_ctl_register_) << std::endl;

    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x7, coincidenceLogicWord);  // setup selection logic
    OtsUDPHardware::write(writeBuffer);
    __CFG_COUT__ << "Selection Logic word is bit: " << std::bitset<16>(coincidenceLogicWord) << std::endl;
    //__CFG_COUT__ << "Selection Logic word is int: " << coincidenceLogicWord << std::endl;

    // NOTE: Consider commenting this out and NOT enabling sig_log during configure (wait until running)
    sel_ctl_register_ |= 1 << 1;  // renable selection logic (i.e. bit-1 <= 1)
    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x6, sel_ctl_register_);  // re-enable selection logic
    OtsUDPHardware::write(writeBuffer);
    ...
    if (optionalLink.getNode("SignalGeneratorEnable").getValue<bool>()) {
      ...
      __CFG_COUT__ << "Configured signal generator with a count of " << sigGenCount
                   << " (0 is continuous output), a high period of " << sigGenHighPer << ", a low period of "
                   << sigGenLowPer << ", and output inversion set to " << sigGenPolarity << std::endl;
      ...
      nimResets_.set(5);     // set bit 5 in resets to 1 to make sure sig gen is held in reset
      nimEnables_.reset(5);  // set bit 5 in enables to 0 to disable sig gen
      __CFG_COUT__ << "Signal Generator disabled" << std::endl;
    }

    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x18000, nimResets_.to_ulong());  // set sig gen in or out of reset
    OtsUDPHardware::write(writeBuffer);
    __CFG_COUT__ << "Nim Resets (after sig gen setup) set to " << nimResets_ << std::endl;

    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x18001, nimEnables_.to_ulong());  // enable or disable sig gen
    OtsUDPHardware::write(writeBuffer);
    __CFG_COUT__ << "Nim Enables (after sig gen setup) set to " << nimEnables_ << std::endl;
  ...
  __CFG_COUT__ << "Checking sel_ctl_register_... Expect sel_ctl_register_ = 0x" << std::hex << sel_ctl_register_
               << std::dec << std::endl;
  ...
  __CFG_COUT__ << "sel_ctl_register_ = 0x" << std::hex << sel_ctl_register_ << std::dec << std::endl;
...
void FENIMPlusInterface::start(std::string runNumber) {
  ...
  OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x6,
  ...
  OtsUDPFirmwareCore::writeAdvanced(
      writeBuffer, 0x18000,
      nimResets_
          .to_ulong());  // reset everything (counters, e.g. sig_log and sig_norm/cms1/cms2 counters, and vetos/ps)
  OtsUDPHardware::write(writeBuffer);
...
void FENIMPlusInterface::stop(void) {
  ...
  OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x6 /*address*/,
                                    (sel_ctl_register_) & (~(1 << 1)));  // disable siglog block
  OtsUDPHardware::write(writeBuffer);
  ...
  if (!optionalLink.isDisconnected()) {
    ...
    if (filename != "DEFAULT" && filename != "") {
      std::string filename = optionalLink.getNode("TriggerCountAtRunStopFilename").getValue<std::string>();
      ...
      if (filename != "DEFAULT" && filename != "") {
        ...
        if (fp) {
          ...
          fprintf(fp, "DAC       Clock-Lock-Loss \t %d 0x%4.4X\n", count, count);
          count = (((readQuadWord >> 24) & 0xF) >> 1) & 1;
          fprintf(fp, "Main      Clock-Lock-Loss \t %d 0x%4.4X\n", count, count);
          count = (((readQuadWord >> 24) & 0xF) >> 2) & 1;
          fprintf(fp, "External  Clock-Lock-Loss \t %d 0x%4.4X\n", count, count);
          count = (((readQuadWord >> 24) & 0xF) >> 3) & 1;
          fprintf(fp, "OutPhase  Clock-Lock-Loss \t %d 0x%4.4X\n", count, count);
  ...
  writeBuffer.resize(0);
  OtsUDPFirmwareCore::writeAdvanced(
      writeBuffer, /*address*/ 0x4,
      /*data*/ 0x33);  // only reset signorm  0x33); //reset output channel blocks synchronously
  OtsUDPHardware::write(writeBuffer);
}
  ...
  try {
    ...
    if (usingOptionalParams) {
      try {
        ...
        __CFG_COUT__ << "Ingore missing SecondsDelayBeforeStartingTriggers field..." << __E__;
      }
    }
    ...
    for (channelCount = 2; channelCount <= 2; --channelCount) {
      enable40MHzMask =
          usingOptionalParams &&
          optionalLink.getNode("EnableClockMaskTriggerOutput" + outChannelNames[channelCount]).getValue<bool>();
      gateChannelVetoSel =
          usingOptionalParams
              ? optionalLink.getNode("VetoSourceTriggerOutput" + outChannelNames[channelCount]).getValue<int>()
              : 0;

      OtsUDPFirmwareCore::writeAdvanced(
          writeBuffer, channelCount == 0 ? 0x4 : (0x18016 + channelCount - 1),
          (enable40MHzMask ? 0x0 : 0x8) | (gateChannelVetoSel <= 1 ? 0 : (1 << 2)));  // unreset output channel block
      OtsUDPHardware::write(writeBuffer);
    }
...
//	__CFG_COUT__ << "clocksOfDelayBetweenTriggers " << clocksOfDelayBetweenTriggers << __E__;
//
//
...
void FENIMPlusInterface::FEMacroGenerateTriggers(__ARGS__) {
  ...
  // launch triggers based on parameters...
  {
    ...
    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x18001, nimEnables_.to_ulong());  // enable or disable sig gen
    OtsUDPHardware::write(writeBuffer);
    __CFG_COUT__ << "Nim Enables set to 0x" << std::hex << nimEnables_ << std::dec << std::endl;
    ...
    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x18005, nimEnables_.to_ulong());  // enable or disable sig gen
    OtsUDPHardware::write(writeBuffer);
    __CFG_COUT__ << "Nim Enables set to 0x" << std::hex << nimEnables_ << std::dec << std::endl;
    ...
    // set bit 5
    nimEnables_.set(5);
    OtsUDPFirmwareCore::writeAdvanced(writeBuffer, 0x18001, numberOfTriggers);  // enable or disable sig gen
    OtsUDPHardware::write(writeBuffer);
    __CFG_COUT__ << "Nim Enables set to 0x" << std::hex << nimEnables_ << std::dec << std::endl;
  }
...
//========================================================================================================================
void FENIMPlusInterface::changeDACLevel(const std::string &channelName, unsigned int dacValue) {
  std::string writeBuffer;
  // 16 bit DAC value
  ...
  //			<< std::endl;
  writeBuffer.resize(0);
  OtsUDPFirmwareCore::writeAdvanced(writeBuffer, /*address*/ 0x0,
